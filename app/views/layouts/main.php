...
<head>
    ...
    <title>Registru Voluntari · Salvamont Zărnești</title>
    ...
    <nav class="navbar navbar-expand-lg">
        <div class="container">
            <a class="navbar-brand d-flex align-items-center gap-2" href="<?= route_url($currentUser ? ($currentUser['role'] === 'admin' ? '?route=admin/dashboard' : '?route=applicant/dashboard') : '?route=login') ?>">
                <img src="<?= asset_url('img/logo-salvamont.svg') ?>" alt="Salvamont Zărnești" height="42">
                <span class="brand-text">Salvamont Zărnești<br><small>Registru voluntari</small></span>
            </a>
            <?php if ($currentUser): ?>
...
<footer class="app-footer text-center py-3">
    <div class="container">
        <small>&copy; <?= date('Y') ?> Salvamont Zărnești — Registru digital de activități ale voluntarilor</small>
    </div>
</footer>
